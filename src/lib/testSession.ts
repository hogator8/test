...
export async function loadSessionForStudent(
  ...
  const { data, error } = await supabase
    .from("test_sessions")
    .select(
      "id, student_id, test_id, status, started_at, submitted_at, total_score, auto_submitted, tests(id, title, time_limit_minutes, leave_detection_enabled, leave_grace_seconds, leave_count_threshold, leave_duration_threshold_seconds, leave_action, leave_warning_message, pause_release_pin)"
    )
    .eq("id", sessionId)
    .maybeSingle();
